...
class Apple:
    ...
    def __get_random_position(self):
        x_position = random.randrange(
            self.__scale, self.__x - self.__scale, self.__scale
        )
        y_position = random.randrange(
            self.__scale, self.__y - self.__scale, self.__scale
        )
        position = [x_position, y_position]

        return (
            position if position not in self.__to_pass else self.__get_random_position()
        )
